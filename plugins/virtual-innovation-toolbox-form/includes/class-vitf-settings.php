...
class VITF_Settings {
    ...
    private $defaults = [
        ...
        'popup_width'           => '420',
        'popup_overlay_color'   => 'rgba(0,0,0,0)',
        'popup_bg_color'        => '#ffffff',
        'popup_box_shadow'      => '0 8px 40px rgba(0,0,0,0.18)',
        'popup_shadow_color'    => '#000000',
        'popup_animation'       => 'slide-up',
        'popup_anim_seconds'    => '0.35',
        'popup_trigger'         => 'on_load',
        'popup_scroll_px'       => '300',
        'trigger_selector'      => '',
        'trigger_selector_type' => 'id',
        'show_on'               => 'all',
        'show_pages'            => [],
        'show_posts'            => [],
        'close_on_submit'       => '1',
    ...
    public function enqueue_admin_assets( $hook ) {
        ...
        wp_enqueue_script( 'vitf-admin', VITF_PLUGIN_URL . 'admin/js/admin.js', [ 'jquery' ], VITF_VERSION, true );
    ...
    public function ajax_save() {
        ...
        $settings['popup_overlay_color']   = sanitize_text_field( $raw['popup_overlay_color'] ?? 'rgba(0,0,0,0)' );
        ...
        $settings['popup_shadow_color']    = sanitize_hex_color( $raw['popup_shadow_color'] ?? '#000000' ) ?? '#000000';
        $settings['popup_animation']       = sanitize_text_field( $raw['popup_animation'] ?? 'slide-up' );
        $raw_sec = floatval( $raw['popup_anim_seconds'] ?? 0.35 );
        $settings['popup_anim_seconds']    = max( 0.1, min( 3.0, $raw_sec ) );
        ...
        $settings['trigger_selector']      = sanitize_text_field( $raw['trigger_selector'] ?? '' );
        $settings['trigger_selector_type'] = sanitize_text_field( $raw['trigger_selector_type'] ?? 'id' );
        ...
        $settings['close_on_submit']       = ( isset( $raw['close_on_submit'] ) && $raw['close_on_submit'] === '1' ) ? '1' : '0';
        $raw_days = isset( $raw['reopen_days'] ) ? trim( $raw['reopen_days'] ) : '-1';
        $settings['reopen_days'] = ( $raw_days === '-1' || intval( $raw_days ) < 0 ) ? '-1' : (string) absint( $raw_days );
